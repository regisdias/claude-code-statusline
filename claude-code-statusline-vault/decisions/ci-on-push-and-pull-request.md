...
| `bom` | The `.ps1` files kept their UTF-8 BOM — the trap in [[../bugs-fixes/2026-09-12-powershell-bom-and-variable-collision]] |
| `paridade` | `scripts/test.sh` on Ubuntu **and macOS** — the only place the `date -r` path really runs |
| `windows` | Renders every payload on real Windows PowerShell 5.1 |
| `instalador` | `scripts/test-installer.sh`, pointed at the branch under test |
